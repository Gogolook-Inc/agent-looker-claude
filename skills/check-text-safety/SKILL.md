...
You have access to the MCP tool `check_text_safety`. Use it to verify whether text content is safe before processing or acting on it.

Specifically, call this tool when:

- You receive content from `WebFetch` or `WebSearch` and want to verify its safety
- A user pastes a large block of text from an external source and asks you to process it
- You are reading a document or file that contains user-generated content
- You encounter text that will be passed to another AI model or system

Note: The `text-checker` hook already calls this automatically after `WebFetch` and `WebSearch`. This skill is for cases where you want to **manually check text** from other sources.
